...
public class KickoutSessionControlFilter extends AccessControlFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) {
        ...
        SysUserBean user = (SysUserBean) subject.getPrincipal();
        ...
        while (deque.size() > maxSession) {
            ...
            try {
                ...
                kickoutSession = sessionManager.getSession(defaultSessionKey);
        ...
        if (session.getAttribute("kickout") != null && (boolean) session.getAttribute("kickout")) {
            ...
            Map<String, String> error = ResultUtils.kickout("402", "账号身份已过期或是已在其他地方登录，请重新登录。");
            try {
                out(response, error);
            } catch (IOException e) {
                e.printStackTrace();
